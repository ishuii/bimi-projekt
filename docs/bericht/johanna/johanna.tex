...
Die visuelle Aufbereitung der komplexen Darstellungen fordern fehlerfrei Interpretierbarkeit, Barrierefreiheit und visuelle Ausgeglichenheit/Neutralität. Dadurch sollen kognitive Verzerrungen durch ungleiche Signalwirkung vermieden werden und die Übersichtlichkeit beibehalten werden. Um dies zu erreichen muss sichergestellt werden, dass die Extrempunkte einer Messreihe visuell mit exakt derselben Intensität wahrgenommen werden, dies wird durch die selbe Leutdichte und Sättigung gewährleistet. Ist dies nicht gewährleistet würde das zu klinischen Fehlinterpetation bezüglich der Relevanz bestimmter Cluster führen. Die Berücksichtigung von Sehschwächen, insbesondere der Rot-Grün-Blindheit wurde für einen höheren Grad der Barrierefreiheit zu erlangen \cite{Crameri2020}.

Diese datenbezogenen Kontraste dürfen zudem nicht mit der Grafikoberfläche konkurrieren. Aus diesem Grund wurde für das gesamte Layout der GUI ein bewusst neutrales, grau-weißes Farbschema gewählt. Diese chromatische Zurückhaltung der Steuerungselemente minimiert die visuelle Ermüdung bei längerer Bildschirmnutzung und stellt sicher, dass der Fokus des Anwenders unverzerrt auf den biologischen Mustern im Grafikpanel liegt \cite{Tufte1983}.

\subsubsection{Farbpaletten}
Für die Wahl der Farbpaletten wurden sowohl theoretische als auch subjektive Farbwahrnehmungsaspekte in Betracht gezogen. Letztendlich wurden vier Farbtabellen ausgewählt, welche für die Heatmap zur Verfügung stehen. Sie werden über die R-Pakete \texttt{RColorBrewer} \cite{HarrowerBrewer2003} und  \texttt{viridis} angesteuert. 
...
\midrule
\texttt{RdYlBu} & Divergierend & Bietet durch das helle, gelbe Zentrum einen neutralen Mittelpunkt für zweiseitige Abweichungen. Die äußeren Maxima entfalten eine visuell gleichwertige Intensität. \\
\addlinespace
\texttt{RdBu} & Divergierend & Ein klassisches, in der Genexpressionsanalyse bewährtes Farbschema. Es liefert maximale Kontraste und verzichtet zur Vermeidung von Rot-Grün-Fehlsichtigkeiten auf Grünanteile. \\
\addlinespace
\texttt{PRGn} & Divergierend & Dient als barrierefreie und kontrastreiche Alternative zu traditionellen Skalen. Die Kombination aus Violett und Grün ist für farbfehlsichtige Personen präzise differenzierbar. \\
\addlinespace
\texttt{viridis} & Sequentiell & Zeichnet sich durch einen absolut linearen Verlauf der Luminanz (Helligkeit) aus. Die visuelle Information bleibt auch bei Konvertierung in Graustufen oder Schwarz-Weiß-Druck vollständig erhalten. \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsubsection{Dendrogramm}
%Überarbeiten
Die Farbgebung der des Dendrogramms orientiert sich an den gewählten Farbpaletten. Allerdings mussten die hellen mittleren Farben ausgeschlossen werden, da sie zu hell für die Darstellung sind. Deshalb gibt es nur 9 Farben, auf die die Labels dann zurück greifen können. Falls es mehr Farben geben sollte wird auf die "default-colors" zurück gegriffen, die von mir zusammengesetllt wurden. 

\begin{verbatim}
default_colors <- c(
    "#0000FF", "#FF6C00", "#005300", "#A100FA", "#B2DF8A", "#FFD300",
    "#0096FF", "#9B4D00", "#00FFD2", "#FDBF6F", "#7B8100", "#960000",
    "#00646B", "#D60072", "#00FF00", "#FF0000", "#540066", "#00A278",
    "#000094", "#FFC0CB"
)
\end{verbatim}
