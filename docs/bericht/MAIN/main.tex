...
\begin{table}[h!]
	...
	\begin{tabular}{l|l}
		...
		Repository- und Package-Erstellung und Verwaltung & Wiktoria \\
	\end{tabular}
\end{table}

Die im Rahmen des Projekts erstellte Anwendung ist als R-Paket \verb*|bimiProjektPaket| verfügbar. Informationen zur Installation und Ausführung des Pakets befinden sich in der dazugehörigen \verb*|README|-Datei. Eine ausführliche Erklärung der wichtigsten Funktionen sowie der einzelnen Aufgabenbereiche erfolgt in den folgenden Kapiteln.
 Die vorliegende Projektarbeit beschreibt sowohl die theoretischen Grundlagen als auch die technische Umsetzung der entwickelten Anwendung.
...
\begin{table}[h!]
	...
	\begin{tabular}{l|l|l}
		...
		Complete-Linkage              & Rosina    & 0,5 Stunden        \\ 
		...
		Berichterstellung (eigenes Kapitel)					& Wiktoria  & 12 Stunden \\
		Bericht zusammenfügen und Korrektur                 & Wiktoria  & 5 Stunden \\
		Folien zusammenfügen und Korrektur 					& Wiktoria  & 1 Stunde \\
		Repository-Pflege 									& Wiktoria  & 3 Stunden \\
		Erstellen einer Library 							& Wiktoria  & 10 Stunden \\
	\end{tabular}
\end{table}
...
Für die weitere Verarbeitung wird der Datensatz anschließend in eine Matrix umgewandelt. Anschließend wird der ganze Inhalt zu numerischen Werten konvertiert. Dies stellt sicher, dass berechenbare Zahlen in der Matrix vorhanden sind. Nicht-numerische Werte, wie Buchstaben, werden dadurch zu fehlenden Einträgen (\texttt{NA}) umgewandelt. So können sie in der darauffolgenden NA-Behandlung mit entfernt werden.
...
Im Projekt wurde eine gemeinsame Funktion für die Berechnung der Distanzmatrix in C++ implementiert. Dafür wurde zunächst ein separates Repository erschaffen und als Paket exportiert. Beim Verlegen des gesamten Projekt in ein Paket erübrigte sich das separate Repository; der C++ Code ist nun also Teil des Pakets \verb*|bimiProjektPaket|. Das Paket \verb*|Rcpp| \cite{Eddelbuettel2026} ermöglicht die Einbindung des C++-Codes in eine R-Umgebung. So kann die C++ Funktion in R ausgeführt werden. Die eigentliche Funktion heißt \verb*|dist_cpp| und bekommt drei Parameter übergeben: eine numerische Matrix \verb*|mat| (den normalisierten Datensatz), einen String \verb*|method| für das gewählte Distanzmaß, und einen Integer \verb*|p|. Letzterer entspricht dem Parameter \(r\) in der Minkowski-Distanz (D3). Die Implementierung in C++ folgt weithin den mathematischen Formeln und ist daher vergleichbar unkompliziert. Vor allem die Einbindung des Codes als separates Paket war jedoch zeitaufwendig und führte zunächst zu einigen Fehlern, die primär die Installation des Pakets betrafen. Nach Verschieben in ein gemeinsames Paket bereitete \verb*|dist_cpp| keine Probleme mehr. 
...
\subsubsection{Herausforderungen bei der Implementierung}
\label{herausforderungen-wiktoria}
Sowohl die Distanzfunktionen als auch die Linkage-Verfahren waren technisch nicht schwierig zu implementieren. Da es sich im groben um reine Mathematik handelt, war das Programmieren entsprechend simpel. Deutlich mehr Zeit nahm das Umsetzen der Skripte in ausführbare Pakete. Für die in C++ geschriebenen Distanzfunktionen wurde zunächst ein eigenes Paket erstellt, was sich im Nachhinein als unnötig kompliziert herausstellte. Da am Ende der gesamte Code in einem Paket zusammengefasst wurde, war eine solche Auslagerung nur für die Distanzfunktionen nicht notwendig. Auch das Erstellen des finalen Pakets \verb*|bimiProjektPaket| nahm viel Zeit ein. Das zentrale Problem hier war, dass erst vergleichsweise spät entschieden wurde, den Code als ein solches Paket zur Verfügung zu stellen. Gleichzeitig waren die R-Skripte ähnlich spät finalisiert. So mussten beispielsweise alle Abhängigkeiten im Nachhinein überprüft und eindeutig gekennzeichnet werden. Im besten Fall hätte dies an die einzelnen Zuständigen delegiert werden können, doch aus Zeitgründen war das nicht mehr sinnvoll. In der Zukunft sollte man deutlich früher als Gruppe die Entscheidung treffen, ob ein Paket erstellt wird. Dann kann auch die Dokumentation entsprechend früh und in einem für Pakete angemessenem Format erfolgen.
...
\subsection{Das Dendrogramm}

Unter einem Dendrogramm versteht man ein \textbf{Baumdiagramm}, welches die hierarchischen Beziehungen zwischen Objekten darstellt und diese in zusammengehörige, ähnliche Gruppen kategorisiert. Es macht somit Zusammenhänge sichtbar, die
...
Am unteren Ende der Baumstruktur liegen die \textbf{Blätter}, die jeweils ein 
einzelnes Untersuchungsobjekt repräsentieren und den Ausgangspunkt der Clusterung 
bilden. \\
...
Der \textbf{Wurzelknoten} des Graphen steht schließlich 
...
Cluster \cite{ivchenko2026, LSE2026}.
...
Cluster-Objekts, das an die Struktur von \code{hclust} angelehnt ist \cite{RCT2026}. Den Ablauf der Clusterung protokolliert die Merge-Matrix. Bei $n$ Objekten hält sie in $n-1$ 
...
\paragraph{Die Datenschicht}

Den Anfang macht die Funktion \code{build\_tree()}, die aus der Merge-Matrix einen 
...
\paragraph{Die Farbschicht und die Renderer}
...
indem sie ein ggplot-Objekt Schicht für Schicht aufbaut \cite{Wickham2016}

\code{plot\_dendro\_plotly()} hingegen erzeugt die interaktive Variante für das Dashboard. Dazu baut die Funktion die Segmente als native Linien-Traces auf und setzt die um 90 Grad gedrehten Beschriftungen als Annotationen \cite{Sievert2020}
...
\subsubsection{Herausforderungen und Entscheidungen}

\paragraph{Flexibilität statt Spezialfälle}
...
\paragraph{Entscheidungen in der Datenschicht}
...
\paragraph{Darstellung von großen Datensätzen}
...
\paragraph{Der ursprüngliche Ansatz und die kurzfristige Umstellung auf plotly}
...
werden Namen etwa an drei Stellen aufgelöst, weil Renderer 
und Panel nach und nach ergänzt wurden. Für die Berechnung der Dendrogramme selbst hätte sich 
...
Weitere Schwierigkeiten zeigten sich in den vereinbarten Deadlines. Da diese nicht eingehalten worden sind kam es zu zeitlichen Verschiebungen im weiteren Projektverlauf. Außerdem wurden kurz vorm Ende Fehler gefunden, diese kurzfristigen Änderungen haben zur großer Verwirrung geführt. 
...
\begin{table}[h]
	...
	\begin{tabular}{|l|p{14cm}|}
		...
		Paket & Aufgabe \\ 
		\hline
		shiny & R-Paket, mit dem man Web-Apps und Dashboards erstellen kann \cite{Chang2026}\\
		\hline
		dipsaus & Add-on-Paket für Shiny, um praktische Funktionen und Verbesserungen hinzuzufügen \cite{Wang2026}\\
		\hline
		...
		\hline
		shinyjs & wichtig für die Verwendung von nützlichen JavaScript-Operationen \cite{Attali2022}\\
		\hline
		bslib & wichtig für die Erstellung von custom bootstrapping-themes, was es einfacher macht Shiny-Apps zu stylen \cite{Sievert2026}\\
		\hline
		bsicons & Paket, wichtig für die Verwendung von Bootstrap Icons in Shiny \cite{Sievert2023}\\
		\hline
		shinyBS & bietet extra interaktive UI-Komponenten für Shiny-Apps wie Modal Pop-ups zum Beispiel \cite{Bailey2026} \\
		\hline
...
\begin{center}
	...
	\begin{minipage}[t]{0.2\textwidth}
		...
		\captionof{figure}{Beispiel des Action Buttons}
		\label{fig:runButton}
	\end{minipage}
...
\begin{table}[H]
	...
	\begin{tabular}{l|l|l}
		...
		Bericht Korrekturlesen               & Johanna   & 2 Stunden    \\ 
